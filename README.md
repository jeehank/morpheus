...
- [`src/services/supabaseClient.ts`](file:///c:/Users/JEEHAN/Downloads/morpheusprep/src/services/supabaseClient.ts) — Supabase database and auth service. Manages user registration (strictly enforcing 1 account per IP address), login, session persistence, email verification, review submission (strictly 1 review per title per user), profanity filtering, report handling, watchlist sync, and admin user management.
